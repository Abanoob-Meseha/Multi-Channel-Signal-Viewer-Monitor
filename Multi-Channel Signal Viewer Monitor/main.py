...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def load(self):#------------------------>>LOAD THE SIGNAL AND PLOT IT<<
        ...
        self.data_lines.append (self.GraphicsView[0].plot(self.x, self.y, pen=self.COLOR_Pen,name=self.LABEL[Channel_ID]))
        self.GraphicsView[0].plotItem.setLabel("bottom", text="Time (ms)")
        self.GraphicsView[0].plotItem.showGrid(True, True, alpha=1)
        self.GraphicsView[0].plotItem.setLimits(xMin=0, xMax=12, yMin=-0.6, yMax=2)
        self.IDX = 0
    ...
    def update_plot_data(self, Channel_ID):#------------>>UPDATE THE VALUES FOR A LIVE SIGNAL<<
        ...
        if self.x[self.IDX] > 0.5:
            self.GraphicsView[0].setLimits(xMin=min(x, default=0),   #get max of x and min to know the limits of graph
                                            xMax=max(x, default=0))     # >> or disable paning over xlimits<<
    ...
    def zoomIn(self):#----------------->>ZOOMIN THE GRAPH<<
        ...
        self.GraphicsView[0].setYRange(yrange[0] / 2, yrange[1] / 2, padding=0)#the range will get smaller >>ZOOMIN<<
        self.GraphicsView[0].setXRange(xrange[0] / 2, xrange[1] / 2, padding=0)

    def zoomOut(self):#---------------->>ZOOMOUT THE GRAPH<<
        xrange, yrange = self.GraphicsView[0].viewRange()
        self.GraphicsView[0].setYRange(yrange[0] * 2, yrange[1] * 2, padding=0)#the range will get bigger >>ZOOMOUT<<
        self.GraphicsView[0].setXRange(xrange[0] * 2, xrange[1] * 2, padding=0)

    
    def SPECTROGRAM(self):#-------------------->>DRAW THE SPECTROGRAM<<
        self.SPECTRO_INDEX=self.ui.SPECTROGRAMS.currentIndex()
        for i in reversed(range(self.ui.verticalLayout_3.count())): #LOOP TO DELETE THE OLD WIDGET>>SPECTROGRAM<< WITH ITS ALL ITEMS
        ...
        MINVALUE=self.ui.verticalSlider_MIN.value()#FOR CONTRAST Vmin
        MAXVALUE=self.ui.verticalSlider_MAX.value()#FOR CONTRAST Vmax
        self.ui.sc_1 = MplCanvas(self.ui.verticalLayoutWidget_2, width=5, height=5, dpi=100)#ADD NEW WIDGET AGAIN AFTER DELETED
    ...
    def speed_up(self):#------------>>INCREASE SIGNAL SPEED<<
        ...
        self.GraphicsView[0].plotItem.setXRange(max(x, default=0) - 0.5, max(x, default=0))
        self.data_lines[Channel_ID].setData(x, y)
        self.Interval=self.Interval-10 #INTERVAL DECREASES 
    ...
    def speed_down(self):#------------>>DECREASE SIGNAL SPEED<<
        ...
        self.GraphicsView[0].plotItem.setXRange(max(x, default=0) - 0.5, max(x, default=0))
        self.data_lines[Channel_ID].setData(x, y)
        self.Interval=self.Interval+10 #INTERVAL INCREASES
        self.Timer[Channel_ID].setInterval(self.Interval)
    ...
    def generate_pdf(self):
        Channel_ID=self.ui.SIGNALS.currentIndex()
        exporter1 = pg.exporters.ImageExporter(self.ui.graphicsView.plotItem)#<GET THE GRAPH IMAGE>
        exporter1.export('signals'+ '.png')#<EXPORT THE IMAGE TO OUR FOLDER PATH>
        self.ui.sc_1.print_png('DSP.png')#<GET SPECTRO IMAGE>
        self.images = ['signals.png']#>>SAVE GRAPH IMG IN ARRAY IF WE NEED TO LOOP
        pdf = FPDF()
        pdf.add_page()#NEW PAGE
        pdf.set_font('Arial', 'B', 15)
        pdf.ln(80)#SPACE IN PAGE
        pdf.cell(0, 0, 'Signals picture', 0, 1, 'C')#NEW CELL
        ...
        pdf.add_page()
        pdf.set_font('Arial', 'B', 15)
        for Channel_ID in range (len(self.SIGNAL_X)):#LOOP TO PRINT ALL READED SIGNAL DATA
            ...
            pdf.ln(10)
            pdf.multi_cell(70, 9, self.x_STRING.describe().to_string(), 1, 0, 'C')#GET and PRINT IMPORATNT GRAPH INFO LIKE MAX,MIN.....
            pdf.ln(10)
